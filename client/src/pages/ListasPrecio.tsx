    ...
    <div className="h-full flex flex-col bg-gray-50 p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Configura modificadores globales de precio</p>
        </div>
        {puedeEditar && (
          <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-100 border-b border-gray-200 sticky top-0 z-10">
              <tr>
                ...
                <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center">Predeterminada</th>
                {puedeEditar && (
            ...
            <tbody className="divide-y divide-gray-100">
              {cargando ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-400">Cargando listas...</td>
                </tr>
              ) : listas.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-400">No hay listas de precios configuradas</td>
                </tr>
              ) : (
                listas.map(lista => (
                  <tr key={lista.id} className="hover:bg-blue-50/50 transition-colors">
                    <td className="p-4 text-sm font-bold text-gray-800">{lista.nombre}</td>
                    <td className="p-4 text-sm text-gray-600">
                      {lista.tipoModificador === 'PORCENTAJE' ? 'Porcentaje (%)' : 'Monto Fijo ($)'}
                    </td>
                    <td className={`p-4 text-sm font-bold ${lista.valor < 0 ? 'text-red-500' : 'text-green-600'}`}>
                      {lista.valor > 0 ? '+' : ''}{lista.valor}
                    </td>
                    <td className="p-4 text-sm text-center">
                      {lista.esPredeterminada ? (
                        <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-bold">Sí</span>
                      ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto">
              <form id="lista-form" onSubmit={handleGuardar} className="flex flex-col gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Tipo de Modificador</label>
                  <select
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  />
                  <label htmlFor="esPredeterminada" className="text-sm font-bold text-gray-700 cursor-pointer">
