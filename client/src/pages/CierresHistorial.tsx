...
const CierresHistorial = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 overflow-hidden print:bg-white print:overflow-visible">
      
      {/* PANTALLA NORMAL (Oculta al imprimir) */}
      <div className="flex flex-col h-full p-6 overflow-hidden print:hidden">
        
        {/* Header y Filtros */}
        <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex-none">
          <div>
            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
              <Wallet className="text-brand-light" size={28} /> Historial de Arqueos (Cierres Z)
            </h1>
            <p className="text-gray-500 text-sm mt-1">Consulta e imprime los cierres pasados</p>
          </div>
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
          {cargando ? (
            <div className="flex-1 flex items-center justify-center text-gray-500">Cargando arqueos...</div>
          ) : cierres.length === 0 ? (
            ...
            <div className="flex-1 overflow-y-auto relative">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50 sticky top-0 border-b border-gray-200 z-10 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
                  <tr>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase">Fecha / Hora Cierre</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase">Caja</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase">Cajero</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase text-right">Efectivo Esperado</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase text-right">Contado</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase text-right">Diferencia</th>
                    <th className="p-4 text-xs font-bold text-gray-500 uppercase text-center w-24">Acciones</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 pb-32">
                  {cierres.map(cierre => {
                    ...
                    return (
                      <tr key={cierre.id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4 font-bold text-gray-600">
                          {cierre.aperturaCaja.caja.nombre} <span className="text-gray-400 font-normal">({cierre.aperturaCaja.caja.prefijo})</span>
                        </td>
                        <td className="p-4 text-sm text-gray-600">{cierre.usuario.nombre}</td>
                        <td className="p-4 text-sm text-right text-gray-500 font-medium">
                        ...
                        <td className={`p-4 text-sm font-extrabold text-right ${difNum < 0 ? 'text-red-600' : difNum > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                        ...
                        <td className="p-4 text-center relative">
                          ...
                          {menuAbierto === cierre.id && (
                            <div className="absolute right-4 top-12 w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-50 py-1 flex flex-col text-left">
                              <button 
                                onClick={() => handleImprimir(cierre, false)}
                                className="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2"
                              >
                                <Printer size={16} className="text-gray-400" /> Reimprimir Ticket Z
                              </button>
                              <button 
                                onClick={() => handleImprimir(cierre, true)}
                                className="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2"
                              >
                                <FileText size={16} className="text-gray-400" /> Z + Resumen de Ventas
                              </button>
                            </div>
                          )}
                        </td>
