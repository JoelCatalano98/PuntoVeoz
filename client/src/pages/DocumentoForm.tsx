    ...
    <div className="h-full flex flex-col p-6 bg-gray-50">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Formulario de emisión de documentos formales</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate(-1)} className="px-4 py-2 border border-gray-300 bg-white text-gray-700 rounded-lg hover:bg-gray-50 font-bold">
            Cancelar
          </button>
          <button onClick={handleGuardar} disabled={guardando} className="px-4 py-2 bg-brand-light text-brand-dark rounded-lg hover:bg-blue-400 font-bold flex items-center gap-2 shadow-sm">
      ...
      <div className="flex flex-1 gap-6 min-h-0">
        ...
        <div className="w-1/3 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-col">
            <h2 className="text-lg font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2 flex items-center gap-2">
              <UserIcon size={18} className="text-gray-400" /> Datos del Cliente
            </h2>
            
            <div className="mb-4">
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Seleccionar Cliente</label>
              <select 
                className="w-full p-2.5 border rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-brand-light outline-none"
            ...
            {clienteSeleccionado && (
              <div className={`p-4 rounded-lg border ${(!clienteSeleccionado.numeroDoc || !clienteSeleccionado.direccion || !clienteSeleccionado.condicionIva) ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
                <div className="font-bold text-gray-800 mb-1">{clienteSeleccionado.razonSocial || clienteSeleccionado.nombre}</div>
                <div className="text-sm text-gray-600 mb-1">
                  <span className="font-semibold">CUIT/DNI:</span> {clienteSeleccionado.numeroDoc || <span className="text-red-500 flex items-center gap-1 inline-flex"><AlertCircle size={12}/>Falta</span>}
                </div>
                <div className="text-sm text-gray-600 mb-1">
                  <span className="font-semibold">Cond. IVA:</span> {clienteSeleccionado.condicionIva || <span className="text-red-500 flex items-center gap-1 inline-flex"><AlertCircle size={12}/>Falta</span>}
                </div>
                <div className="text-sm text-gray-600">
                  <span className="font-semibold">Dirección:</span> {clienteSeleccionado.direccion || <span className="text-red-500 flex items-center gap-1 inline-flex"><AlertCircle size={12}/>Falta</span>}
                </div>
                
                {(!clienteSeleccionado.numeroDoc || !clienteSeleccionado.direccion || !clienteSeleccionado.condicionIva) && (
                  <div className="mt-3 text-xs text-red-600 font-bold bg-red-100 p-2 rounded">
          ...
          {/* TOTALES */}
          <div className="bg-brand-dark rounded-xl shadow-sm border border-brand-dark p-6 flex flex-col justify-center text-white mt-auto">
            <h2 className="text-sm font-bold uppercase tracking-wider text-brand-light/80 mb-1">Total del Documento</h2>
            <div className="text-4xl font-black font-mono tracking-tight">${totalGral.toFixed(2)}</div>
            <div className="text-sm mt-4 pt-4 border-t border-white/10 flex justify-between">
              <span className="text-brand-light">Ítems Totales:</span>
        ...
        <div className="w-2/3 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {/* BUSCADOR PRODUCTOS */}
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex gap-4" ref={searchProdRef}>
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
              <input
                type="text"
                placeholder="Buscar por nombre o código de barras para agregar..."
                className="w-full pl-10 pr-4 py-3 bg-white border border-gray-300 rounded-lg focus:outline-none focus:border-brand-light focus:ring-1 focus:ring-brand-light shadow-sm text-base"
                  ...
                  {productosResult.map((p) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-gray-500 font-mono">{p.codigoBarras || 'Sin código'}</div>
                      </div>
                      <div className="font-bold text-brand-dark">${Number(p.precioVenta).toFixed(2)}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-lg font-bold">Sin productos</p>
                <p className="text-sm">Buscá y agregá productos a la lista</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-white shadow-sm border-b border-gray-100">
                  <tr>
                    ...
                    <th className="p-3 text-xs font-bold text-gray-500 uppercase text-center w-32">Cant.</th>
                    <th className="p-3 text-xs font-bold text-gray-500 uppercase text-right w-28">Precio U.</th>
                    <th className="p-3 text-xs font-bold text-gray-500 uppercase text-right w-28">Subtotal</th>
                    <th className="p-3 text-xs font-bold text-gray-500 uppercase text-center w-16"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      <td className="p-3">
                        <div className="font-bold text-sm text-gray-800">{item.producto.nombre}</div>
                        <div className="text-xs text-gray-500 font-mono">{item.producto.codigoBarras || 'S/N'}</div>
                      </td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center border rounded-lg overflow-hidden w-24 mx-auto bg-white">
                          <button 
                          ...
                          <input 
                            type="number" 
                            className="w-10 text-center font-bold text-sm outline-none border-x py-1"
                            value={item.cantidad}
                      ...
                      <td className="p-3 text-right text-sm font-medium text-gray-600">${Number(item.producto.precioVenta).toFixed(2)}</td>
                      <td className="p-3 text-right text-sm font-bold text-brand-dark">${item.subtotal.toFixed(2)}</td>
                      <td className="p-3 text-center">
                        <button 
