    ...
    <div className="h-full flex flex-col bg-gray-50 dark:bg-slate-900 p-6 transition-colors duration-200">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">Gestión de proveedores del comercio</p>
        </div>
        {puedeEditar && (
          <button 
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-100 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 sticky top-0 z-10">
              <tr>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Razón Social</th>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">CUIT</th>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Teléfono</th>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Email</th>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Dirección</th>
                <th className="p-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Cond. IVA</th>
                {puedeEditar && (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50">
              {cargando ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-gray-400 dark:text-slate-500">Cargando proveedores...</td>
                </tr>
              ) : proveedores.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-gray-400 dark:text-slate-500">No hay proveedores registrados</td>
                </tr>
              ) : (
                proveedores.map(prov => (
                  <tr key={prov.id} className="hover:bg-blue-50/50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="p-4 text-sm font-bold text-gray-800 dark:text-slate-200">{prov.razonSocial}</td>
                    <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{prov.cuit || '-'}</td>
                    <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{prov.telefono || '-'}</td>
                    <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{prov.email || '-'}</td>
                    <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{prov.direccion || '-'}</td>
                    <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{prov.condicionIva || '-'}</td>
                    {puedeEditar && (
                      <td className="p-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] transition-colors duration-200">
            ...
            <div className="p-6 overflow-y-auto">
              <form id="proveedor-form" onSubmit={handleGuardar} className="flex flex-col gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">CUIT</label>
                  <input
                    type="text"
                    className="w-full p-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 rounded-lg focus:outline-none focus:border-brand-light focus:ring-1 focus:ring-brand-light placeholder-gray-400 dark:placeholder-slate-500 transition-colors"
                    value={formData.cuit}
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Teléfono</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-1">Condición frente al IVA</label>
                  <select
                    className="w-full p-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 rounded-lg focus:outline-none focus:border-brand-light focus:ring-1 focus:ring-brand-light transition-colors"
