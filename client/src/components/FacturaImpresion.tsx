...
export const FacturaImpresion: React.FC<FacturaImpresionProps> = ({ compra }) => {
  return (
            ...
            <tbody>
              {compra.detalles.map((detalle, idx) => (
                <tr key={idx} className="border-b border-dashed border-gray-300">
                  ...
                  <td className="py-1 px-2 text-right">{Number(detalle.cantidad).toFixed(2)}</td>
                  <td className="py-1 px-2 text-right">${Number(detalle.precioCosto).toFixed(2)}</td>
                  <td className="py-1 px-2 text-right font-bold">${Number(detalle.subtotal).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
